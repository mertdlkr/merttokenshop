...
actor TokenMarketplace {
    // Token ve cüzdan için tip tanımlamaları
    ...
    // Satış işlemleri için tip tanımlamaları
...
public type Transaction = {
    ...
    // Durumları saklayan değişkenler
    ...
    // Sistem başlangıç ve upgrade işlemleri
    ...
    // Yeni eklenen doğrudan market satış fonksiyonu
    public shared(msg) func sellToMarket(tokenId: TokenId, amount: Nat) : async Result.Result<(), Text> {
        ...
        switch(tokens.get(tokenId)) {
            ...
            case (?token) {
                // Kullanıcının cüzdanından tokenları çıkar
                _removeFromWallet(msg.caller, tokenId, amount);

                // Market supply'ı güncelle
    ...
    public shared(msg) func purchaseSaleOrder(orderId: Nat) : async Result.Result<(), Text> {
        switch(saleOrders.get(orderId)) {
            case null {
                ...
                #err("Satış emri bulunamadı")
            };
            case (?order) {
                if (order.seller == msg.caller) {
                    return #err("Kendi satış emrinizi satın alamazsınız");
                };
    ...
    public shared(msg) func cancelSaleOrder(orderId: Nat) : async Result.Result<(), Text> {
        switch(saleOrders.get(orderId)) {
            ...
            case (?order) {
                if (order.seller != msg.caller) {
                    return #err("Sadece satış emri sahibi iptal edebilir");
                };
    ...
    // Transactions'ları timestamp'e göre sırala (en yeniden en eskiye)
    ...
    transactionArray
        ...
        for (item in walletItems.vals()) {
            // Sadece pozitif bakiyesi olan tokenları işle
            if (item.balance > 0) {
